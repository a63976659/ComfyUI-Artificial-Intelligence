...
1. 一次性录音: 前端采集麦克风 -> 编码 WAV 上传 input 目录 -> 文件名写入"录音文件"参数，
   本节点在图执行时读取并输出 AUDIO，可连接实时翻译节点的音频输入。
...
class 录音_Node:
    ...
    DESCRIPTION = (
        "麦克风录音节点：点击\"🎙️ 开始录音\"可一次性录音上传 (需浏览器授权)，输出 AUDIO 连接实时翻译节点。"
    ...
    def load_record(self, 录音文件):
        ...
        file_path = os.path.join(folder_paths.get_input_directory(), 录音文件)
